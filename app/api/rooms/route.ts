...
export async function POST(req: Request) {
  try {
    ...
    if (is_private && (!password || password.length < 4)) {
      return new NextResponse("Password must be at least 4 characters.", {
        status: 400,
      });
    }
    ...
    const password_hash =
      is_private && password ? await hashPassword(password) : null;
    ...
    if (msg.includes("SCHEMA_INIT_FAILED")) {
      return new NextResponse(`Database schema initialization failed: ${msg}`, {
        status: 500,
      });
    }
    if (msg.includes("duplicate key") || msg.includes("unique constraint")) {
      return new NextResponse("Slug already exists. Choose another.", {
        status: 409,
      });
    }
