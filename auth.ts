...
function getSecret() {
  ...
  const secret = process.env.STACK_SECRET_SERVER_KEY || process.env.POSTGRES_PASSWORD || "fallback-secret"
  return secret
}
...
async function hmacSHA256(keyString: string, data: string) {
  const key = await crypto.subtle.importKey("raw", enc.encode(keyString), { name: "HMAC", hash: "SHA-256" }, false, [
    "sign",
  ])
...
export async function signRoomToken(params: { slug: string; password_hash: string | null }) {
  const secret = getSecret()
